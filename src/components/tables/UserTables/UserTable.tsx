    ...
    async function fetchData() {
      ...
      try {
        ...
        const merged = usersData.map((user) => {
          const matchedVehicle = vehiclesData.find((v) => v.user_id === user.id);
          return {
            ...user,
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {filteredUsers.map((user, index) => (
              <TableRow key={user.id} className={`py-5 ${index % 2 !== 1 ? "bg-gray-200 dark:bg-gray-700" : ""} hover:bg-gray-100 dark:hover:bg-gray-600`}>
                ...
                <TableCell className="py-4 text-gray-800 text-theme-sm dark:text-white/90">{user.student_id}</TableCell>
                <TableCell className="py-4 text-gray-800 text-theme-sm dark:text-white/90">{user.plate}</TableCell>
