...
console.log("Parsed maxCapacity:", parseInt(maxCapacity, 10));
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl mx-auto p-6 bg-white shadow-xl rounded-2xl space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          <Label htmlFor="name">Nama Tempat Parkir<span className="text-error-500">*</span></Label>
          <Input id="name" placeholder="Masukkan nama tempat parkir" value={name} onChange={(e) => setName(e.target.value)}  />
        </div>

        <div>
          <Label htmlFor="capacity">Maksimal Kapasitas<span className="text-error-500">*</span></Label>
  ...
  <div key={day} className="border p-5 rounded-xl bg-white shadow-sm space-y-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-center">
      ...
      <DatePicker
        selected={
    schedules[day]?.open_time
      ? toZonedTime(schedules[day]?.open_time, jakartaTimezone)
      : null
  }
        onChange={(date) =>
          setSchedules((prev) => {
  ...
  popperClassName="react-datepicker-popper"
      ...
      <DatePicker
        selected={
    schedules[day]?.closed_time
      ? toZonedTime(schedules[day]?.closed_time, jakartaTimezone)
      : null
  }
        onChange={(date) =>
          setSchedules((prev) => {
      ...
      {!isActive && (
        <div>
          ...
          <Input
            ...
            onChange={(e) => setInactiveDescription(e.target.value)}
            className="mt-1 w-full rounded-lg border-gray-300 shadow-sm focus:ring-brand-500 focus:border-brand-500"
          />
        </div>
      )}
